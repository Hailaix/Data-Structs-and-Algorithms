...
describe("Shortest Path", function() {
  it("should return an array of the nodes between start and target", function() {
    let graph = new Graph();
    let S = new Node("S");
    let P = new Node("P");
    let U = new Node("U");
    ...
    expect(graph.shortestPath(X,X)).toEqual(["X"]);
    expect(graph.shortestPath(S,V)).toEqual(["S","U","V"]);
    expect(graph.shortestPath(S,Y)).toEqual(["S","P","X","Y"]);
    const result = JSON.stringify(graph.shortestPath(S,T));
    const validResult = result === JSON.stringify(["S","U","V","W","T"]) 
    || result === JSON.stringify(["S","P","Q","R","T"]);
    expect(validResult).toBe(true);
  });
  it("should return undefined if there is no path", function() {
    const graph = new Graph();
    const S = new Node("S");
    const P = new Node("P");
    expect(graph.shortestPath(S,P)).toBeUndefined();
  })
});